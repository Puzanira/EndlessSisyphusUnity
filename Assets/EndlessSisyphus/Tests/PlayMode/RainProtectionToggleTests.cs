...
namespace EndlessSisyphus.Tests
{
    ...
    public class RainProtectionToggleTests
    {
        ...
        /// <summary>
        /// Цвет бейджа защиты. Основательница забраковала красный живым кадром («странно она
        /// красным смотрится — не очень»), и заодно вскрылось: два состояния бейджа просят
        /// РОВНО одно действие (снять защиту), а покрашены были по-разному. Тест пришпиливает
        /// оба решения, чтобы следующая сессия не «починила» их обратно.
        /// </summary>
        [UnityTest]
        public IEnumerator RainGuardBadge_BothTakeOffStates_ShareOneBrightColor_NoRed()
        {
            yield return null;   // класс поднимается через UnitySetUp — остаёмся в его режиме
            Color needsAction = GameUI.RainGuardColor(true);
            Color allGood = GameUI.RainGuardColor(false);

            Assert.AreEqual(new Color(1f, 0.8f, 0.2f), needsAction,
                "«Дождя нет…» и «Дождь прошёл…» просят одно и то же — и красятся одним " +
                "золотом игры (заголовок, рекорд, подсказка крутилки)");
            Assert.AreEqual(new Color(0.49f, 0.94f, 0.82f), allGood,
                "«ЗАЩИТА ОТ ДОЖДЯ — ВКЛ» остаётся бирюзовой: «всё правильно» обязано " +
                "отличаться от «нужно действие»");
            Assert.AreNotEqual(needsAction, allGood,
                "свести все три состояния к одному цвету нельзя — игрок перестанет видеть, " +
                "включилась защита или нет");

            // Бейдж висит у нижнего края, где под ним чаще всего коричневая скала — именно там
            // терялся красный (светлота ≈0.40 против ≈0.32 у скалы). Требуем от цвета действия
            // светлоту, при которой он остаётся читаемым на этом фоне, а не «просто другой».
            float luma = 0.2126f * needsAction.r + 0.7152f * needsAction.g + 0.0722f * needsAction.b;
            Assert.Greater(luma, 0.6f,
                "цвет «нужно действие» обязан быть заметно светлее пейзажа под бейджем " +
                "(светлота вышла " + luma + ")");
        }
    }
}
